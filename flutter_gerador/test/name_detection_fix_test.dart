...
void main() {
  group('🔍 Teste de Detecção de Nomes - Fix Capitalização', () {
    ...
    test('❌ NÃO deve detectar substantivos abstratos como nomes', () {
      ...
      expect(
        names.contains('Iniciativa'),
        false,
        reason: 'Iniciativa é substantivo abstrato',
      );
      expect(names.contains('Quero'), false, reason: 'Quero é verbo');
      expect(names.contains('Lembre'), false, reason: 'Lembre é verbo');
      expect(
        names.contains('Oferta'),
        false,
        reason: 'Oferta é substantivo comum',
      );
      ...
      expect(
        names.contains('Proatividade'),
        false,
        reason: 'Proatividade é substantivo abstrato',
      );
      expect(
        names.contains('Liderança'),
        false,
        reason: 'Liderança é substantivo abstrato',
      );
      expect(
        names.contains('Campanha'),
        false,
        reason: 'Campanha é substantivo comum',
      );
    ...
    test('❌ NÃO deve detectar instituições/locais como nomes', () {
      ...
      expect(
        names.contains('Municipal'),
        false,
        reason: 'Municipal é qualificador',
      );
      expect(
        names.contains('Escola Municipal'),
        false,
        reason: 'Escola Municipal é instituição',
      );
      expect(
        names.contains('Hospital'),
        false,
        reason: 'Hospital é instituição',
      );
      expect(
        names.contains('Central'),
        false,
        reason: 'Central é qualificador',
      );
      expect(
        names.contains('Hospital Central'),
        false,
        reason: 'Hospital Central é instituição',
      );
      expect(
        names.contains('Prefeitura'),
        false,
        reason: 'Prefeitura é instituição',
      );
      expect(
        names.contains('Prefeitura Municipal'),
        false,
        reason: 'Prefeitura Municipal é instituição',
      );
      expect(
        names.contains('Sonho'),
        false,
        reason: 'Sonho é substantivo comum',
      );
      expect(names.contains('Grande'), false, reason: 'Grande é adjetivo');
      expect(
        names.contains('Sonho Grande'),
        false,
        reason: 'Sonho Grande pode ser nome de lugar',
      );
    ...
    test('✅ DEVE detectar nomes reais no meio de frases', () {
      ...
      expect(
        names1.contains('Cecília'),
        true,
        reason: 'Cecília deveria ser detectado',
      );
      expect(
        names2.contains('Carlos'),
        true,
        reason: 'Carlos deveria ser detectado',
      );
      expect(
        names3.contains('Beatriz'),
        true,
        reason: 'Beatriz deveria ser detectado',
      );
      expect(
        names4.contains('Costa'),
        true,
        reason: 'Costa deveria ser detectado',
      );
      expect(
        names5.contains('Elza'),
        true,
        reason: 'Elza deveria ser detectado',
      );
    });
    ...
    test('🔍 Debug: verificar sufixos abstratos', () {
      ...
      expect(
        lower1.endsWith('ade'),
        false,
        reason: 'iniciativa termina com "iva", não "ade"',
      );
      expect(
        lower1.endsWith('iva'),
        true,
        reason: 'iniciativa termina com "iva"',
      );
      ...
      expect(
        lower2.endsWith('idade'),
        true,
        reason: 'proatividade termina com "idade"',
      );
      ...
      expect(
        lower3.endsWith('ança'),
        true,
        reason: 'liderança termina com "ança"',
      );
    });
  });
}
