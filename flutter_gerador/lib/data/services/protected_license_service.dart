...
class ProtectedLicenseService {
  ...
  static Future<Directory> _getDocumentsDirectory() async {
    if (Platform.isWindows) {
      return Directory('${Platform.environment['USERPROFILE']}\\Documents\\FlutterGerador');
    }
  ...
  static Future<Map<String, dynamic>> _checkDeviceProtection(String licenseKey) async {
  ...
  // Ativa dispositivo
  static Future<void> _activateDevice(String licenseKey, String deviceId) async {
  ...
  static Future<LicenseModel?> incrementUsage(String licenseKey) async {
    ...
    // Atualiza último uso
    if (_globalActivations.containsKey(licenseKey)) {
      _globalActivations[licenseKey]!['lastUsed'] = DateTime.now().toIso8601String();
    }
