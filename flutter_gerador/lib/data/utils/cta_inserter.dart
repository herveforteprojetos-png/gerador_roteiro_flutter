...
class CtaInserter {
  ...
  static List<CtaItem> _sortCtasByPosition(
    List<CtaItem> ctas,
    int paragraphCount,
  ) {
  ...
  static int _calculateInsertionPoint(
    ...
    final targetParagraphIndex =
        ((paragraphs.length - 1) * targetPercentage / 100).round();
  ...
  static String _insertAtPosition(
    String original,
    String toInsert,
    int position,
  ) {
  ...
  static List<String> validateCtaPositioning(List<CtaItem> ctas) {
    ...
    for (final cta in ctas) {
      final position = _getPositionPercentage(
        cta,
        100,
      ...
      for (final existingPosition in positions) {
        if ((position - existingPosition).abs() < 10) {
          warnings.add(
            'CTAs "${cta.title}" podem estar muito próximos (posição ${position.toInt()}%)',
          );
        }
      }
